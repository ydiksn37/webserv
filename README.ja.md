...
Webserv（webサーバー）の再実装。
Webservは、C++98で記述された小規模なHTTP/1.1サーバーです。
ネットワークプログラミング、リクエストの解析、ルーティング、およびレスポンス生成というコアコンセプトに焦点を当てつつ、本番環境のWebサーバーに期待される動作のサブセットを実装しています。
